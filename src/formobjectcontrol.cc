...
QTreeWidgetItem *FormObjectControl::createCpointsetTreeItem(CPointset *cpoint_set) {
  auto cpointset_item = new QTreeWidgetItem(
          ...
          QStringList() << QString::fromStdString(cpoint_set->getName()) << QString()
                        << QString::number(cpoint_set->getNbCpoints()),
...
void FormObjectControl::on_add_cpointset_clicked(bool) {
  ...
  auto item = new QTreeWidgetItem(form.cpoints_set_treewidget,
                                  QStringList() << QString::fromStdString(new_pointset->getName()) << QString()
                                                << QString::number(new_pointset->getNbCpoints()), 0);
...
void FormObjectControl::delete_cpointsets(bool need_confirmation) {
  ...
  if (items.size() == 1) {
    ...
    QMessageBox::StandardButton reply;
    if (need_confirmation)
      reply = QMessageBox::question(this, "Delete cpointset",
                                    "Do you really want to delete " + QString::number(items.size()) + " cpointsets ?",
                                    QMessageBox::Yes | QMessageBox::No);
    else reply = QMessageBox::Yes;
    if (reply == QMessageBox::Yes) {
      for (auto item: items) {
...
void FormObjectControl::delete_cpoints(bool need_confirmation) {
  ...
  if (items.size() == 1) {
    ...
    QMessageBox::StandardButton reply;
    if (need_confirmation)
      reply = QMessageBox::question(this, "Delete cpoints",
                                    "Do you really want to delete cpoints \"" + cpoint_name + "\"?",
                                    QMessageBox::Yes | QMessageBox::No);
    else reply = QMessageBox::Yes;
...
void FormObjectControl::selectTreeWidgetItem(int cpoint_id) {
  auto item_matched = form.cpoints_set_treewidget->findItems(QString::number(cpoint_id),
