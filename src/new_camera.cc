...
void BaseCamera::setPosition(vec3 pos) {
  m_position = pos;
  m_matrix_clean = false;
}
...
quat BaseCamera::fromMat34(mat4 m)
{
	quat q;

	q.w = sqrt(fmax(0, 1 + m[0][0] + m[1][1] + m[2][2])) / 2;
	q.x = sqrt(fmax(0, 1 + m[0][0] - m[1][1] - m[2][2])) / 2;
	q.y = sqrt(fmax(0, 1 - m[0][0] + m[1][1] - m[2][2])) / 2;
	q.z = sqrt(fmax(0, 1 - m[0][0] - m[1][1] + m[2][2])) / 2;
...
void NewCamera::setPosition(vec3 pos){
  m_vr_camera->setPosition(pos);
}
