/*
     ...
     $Id: fntTXF.cxx,v 1.1 2010/10/25 09:15:02 jcl Exp $
*/
...
#else
  QFile infile(QString((std::string(fname)).c_str()));
  if (!infile.open(QIODevice::ReadOnly)) {
    ulSetError ( UL_WARNING,
      "QT >> fntLoadTXF: Failed to open '%s' for reading.", fname ) ;
    return FNT_FALSE ;
  }
...
#else
  if (infile.read((char *)magic,4*sizeof(unsigned char))!=4*sizeof (unsigned char))
#endif
  {
    ulSetError ( UL_WARNING,
...
#else
        if (infile.read((char *)orig,ntexels)!= ntexels )
#endif
        {
          ulSetError ( UL_WARNING,
...
#else
        if (infile.read((char *)texbitmap,stride * h)!= stride * h )
#endif
        {
          delete [] texbitmap ;
          ulSetError ( UL_WARNING,
